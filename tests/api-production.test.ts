...
describe('ProductionService - API Integration', () => {
  ...
  it('should create a BOM snapshot when planOrder is called (Forecast integration regression)', async () => {
    const sqlite = createInMemoryDb();
    await setupDb(sqlite);
    const db = sqlite as unknown as D1Database;
    const ctx = createMockContext(1);
    ...
    const bLine = await createBusinessLine(db as any, ctx, { name: 'Panadería', code: 'PAN2' });
    const store = await createStore(db as any, ctx, { name: 'Tienda BOM', code: 'TB' });
    ...
    // planOrder is the canonical path used by Forecast (via ProductionService.planOrder)
    const order = await prodService.planOrder(store.id, bLine.id, [
      { productId: pan.id, quantity: 10 },
    ]);

    // Verify BOM snapshot was persisted
    const { getProductionOrderBOM } = await import('../worker/db/production.repositories');
    const bom = await getProductionOrderBOM(db as any, ctx, order.id);

    expect(bom.length).toBeGreaterThan(0);
    expect(bom[0].productId).toBe(harina.id);
    // 10 panes / 5 rendimiento = factor 2; factor * 1.5kg = 3kg de harina
    expect(bom[0].plannedQuantity).toBeCloseTo(3, 5);
    expect(bom[0].unit).toBe('u');
  });
});
