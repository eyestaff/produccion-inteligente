    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        >
          ...
          <ul style={{ margin: 0, paddingLeft: '1.2rem', color: '#991b1b', fontSize: '0.9rem' }}>
            {data.criticalStock.length > 0 && (
              <li>Hay {data.criticalStock.length} productos con stock crítico (bajo el mínimo).</li>
            )}
            {data.wasteToday > 0 && (
              <li>Se han registrado {data.wasteToday} unidades de mermas hoy.</li>
            )}
          </ul>
        </div>
      )}
      ...
      >
        ...
        >
          ...
          <strong className="kpi-value" style={{ color: '#10b981' }}>
            {loading ? '…' : (data?.productionToday ?? 0)}{' '}
            <span style={{ fontSize: '1rem', fontWeight: 'normal', color: 'var(--muted)' }}>
              uds
            </span>
          </strong>
          <p className="kpi-detail">
            {loading ? '' : `${data?.ordersToday ?? 0} órdenes completadas`}
          </p>
        </article>
        <article
          className="kpi-card"
        ...
        >
          ...
          <strong
            className="kpi-value"
            style={{ color: (data?.pendingOrders ?? 0) > 0 ? '#f59e0b' : 'inherit' }}
          >
        ...
        >
          ...
          <strong
            className="kpi-value"
            style={{ color: (data?.pendingPurchases ?? 0) > 0 ? '#3b82f6' : 'inherit' }}
          >
            {loading ? '…' : (data?.pendingPurchases ?? 0)}
          </strong>
        ...
        <article className="kpi-card">
          ...
          <strong
            className="kpi-value"
            style={{ color: (data?.wasteToday ?? 0) > 0 ? '#ef4444' : 'inherit' }}
          >
            {loading ? '…' : (data?.wasteToday ?? 0)}
          </strong>
      ...
      >
        ...
        <div className="card">
          ...
          {loading ? (
            ...
            <div
              style={{ display: 'flex', alignItems: 'flex-end', gap: '0.5rem', height: '120px' }}
            >
                ...
                >
                  ...
                  >
                    <div
                      style={{
                        width: '100%',
                        height: `${Math.max(4, (v / maxWeekly) * 100)}px`,
                        background: 'var(--text)',
                        borderRadius: '4px 4px 0 0',
...
export function ProductionPage() {
  return (
    <div className="page-grid">
      <PageCard
        title="Programación"
        description="Planificación de tareas y turnos del equipo."
        accent="#6366f1"
      />
      <PageCard
        title="Rendimiento"
        description="Comparativa del avance actual frente a la meta."
        accent="#8b5cf6"
      />
      <EmptyState
        title="Producción pendiente"
        description="La lógica de negocio se añadirá en una fase posterior."
      />
    </div>
  );
}
  ...
  return (
    <div className="page-grid">
      <PageCard
        ...
        description="Visión general de materiales y recursos disponibles."
        accent="#0f766e"
      />
      <PageCard
        title="Movimientos"
        description="Historial de entradas y salidas por almacén."
        accent="#ec4899"
      />
      <EmptyState
        title="Sin datos todavía"
        description="Los registros reales se integrarán cuando se conecte la fuente de información."
      />
    </div>
  );
}
...
export function ConfigurationPage() {
  ...
  const handleSeed = async () => {
    if (
      !window.confirm(
        '¿Seguro que quieres borrar los datos actuales de la empresa y generar un entorno de demostración?',
      )
    ) {
      return;
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      <div className="page-grid">
        <PageCard
          title="Preferencias"
          description="Ajuste de idioma, notificaciones y visualización."
          accent="#475569"
        />
        <PageCard
          title="Integraciones"
          description="Conexiones futuras con sistemas de negocio y servicios externos."
          accent="#0ea5e9"
        />
        <EmptyState
          title="Configuración inicial"
          description="Este espacio recoge los parámetros del sistema sin afectar la arquitectura actual."
        />
      </div>
      ...
      >
        ...
        <p style={{ margin: '0 0 1rem', color: '#6b7280', fontSize: '0.9rem' }}>
          Esta acción <strong>eliminará todos los datos de la empresa actual</strong> y generará un
          escenario completo de panadería (productos, recetas, inventario histórico y órdenes) para
          demostrar el sistema de Forecast.
        </p>
        <button
          onClick={handleSeed}
          disabled={seeding}
          style={{
            padding: '0.75rem 1.5rem',
            background: seeding ? '#fca5a5' : '#ef4444',
            color: 'white',
            border: 'none',
            borderRadius: '8px',
            cursor: seeding ? 'not-allowed' : 'pointer',
