      ...
      <div style={{ padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
        <div style={{ flex: 1, cursor: 'pointer' }} onClick={() => setExpanded(!expanded)}>
          ...
          <div style={{ fontSize: '0.8rem', color: 'var(--muted)', marginTop: '0.25rem' }}>
            Sugerido: <strong>{rec.suggestedQuantity}</strong> (Base:{' '}
            {rec.metrics.consumption30d > 0 ? (rec.metrics.consumption30d / 30).toFixed(1) : 0}/día)
          </div>
        </div>

        <div
          style={{ textAlign: 'right', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}
        >
          <label
            style={{
              fontSize: '0.75rem',
  ...
  return (
    ...
    >
      ...
      {data.risks.length > 0 && (
        ...
        >
          <h3 style={{ margin: '0 0 1rem', fontSize: '1.1rem', color: '#111827' }}>
            Riesgos Detectados
          </h3>
          <div
            style={{
      ...
      {history.length > 0 && (
        <div
          ...
          style={{ padding: 0, overflow: 'hidden', marginTop: '2rem' }}
        >
          <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid var(--border)' }}>
            <h3 style={{ margin: 0, color: 'var(--text)', fontSize: '1.1rem' }}>
              Histórico de Previsiones (Desviaciones)
            </h3>
          </div>
                  ...
                  {history.map((h) => (
                    <tr key={h.id}>
                      ...
                      <td className="text-right" style={{ color: 'var(--muted)' }}>
                        {Number(h.suggestedQuantity).toFixed(2)}
                      </td>
                      <td className="text-right" style={{ fontWeight: 600 }}>
                        {Number(h.adjustedQuantity).toFixed(2)}
                      </td>
                    </tr>
                  ))}
                </tbody>
