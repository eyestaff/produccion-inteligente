...
export function ProductionDashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const filteredOrders = ((fetchedOrders as unknown as ProductionOrderEnriched[]) || []).filter(
        (o) => o.storeId === selectedStoreId,
      );
  ...
  const handleAction = async (
    ...
    try {
      ...
      const msgs = {
        start: 'Producción iniciada',
        complete: 'Producción completada — Stock actualizado automáticamente',
  ...
  const getProductLabel = (o: ProductionOrderEnriched) => {
    if (o.items && o.items.length > 0) {
      return o.items
        .map((i) => `${i.productName || 'Producto #' + i.productId} (×${i.quantity})`)
        .join(', ');
    }
    ...
    >
          ...
          {orders.length > 0 && (
            <button
              onClick={() => {
                ...
                exportToCsv(
                  'ordenes-produccion',
                  ['Orden', 'Producto(s)', 'Tienda', 'Estado'],
                  rows,
                );
              }}
      ...
      <div className="kpi-grid">
        ...
        <article className="kpi-card">
          ...
          <strong className="kpi-value" style={{ color: '#f59e0b' }}>
            {kpis?.inProgress || 0}
          </strong>
        </article>
        <article className="kpi-card">
          ...
          >
            {suggestions.map((s) => (
              ...
              >
                ...
                <div
                  style={{ fontSize: '0.875rem', color: 'var(--muted)', marginBottom: '0.75rem' }}
                >
                  Producir: <strong style={{ color: 'var(--text)' }}>{s.suggestedQuantity}</strong>{' '}
                  uds
                </div>
                <button
                        ...
                        <td className="text-center">
                          ...
                          >
                            {o.status === 'planned' && (
                              <button
                                disabled={isLoading}
                                onClick={() => handleAction(o.id, o.storeId, 'start')}
                                className="btn-primary"
                            ...
                            {(o.status === 'planned' || o.status === 'in_progress') && (
                              <button
                                disabled={isLoading}
                                onClick={() => handleAction(o.id, o.storeId, 'cancel')}
                                style={{
                                  cursor: 'pointer',
