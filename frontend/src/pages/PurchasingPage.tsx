          ...
          {DISCARD_REASONS.map((r) => (
            <label
              ...
              style={{
                ...
                border: `1px solid ${selected === r ? '#2563eb' : '#e5e7eb'}`,
                borderRadius: '8px',
                cursor: 'pointer',
                background: selected === r ? '#eff6ff' : 'white',
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
            <span style={{ fontWeight: 700, fontSize: '1rem', color: meta.color }}>
              {need.productName}
            </span>
            <span
              style={{
            ...
            {need.daysOfStockRemaining !== null && need.daysOfStockRemaining <= 2 && (
              ...
              >
                ⏱{' '}
                {need.daysOfStockRemaining === 0
                  ? 'Se agota hoy'
                  : `${need.daysOfStockRemaining}d restante`}
              </span>
            )}
          </div>
          <div style={{ display: 'flex', gap: '1.25rem', marginTop: '0.25rem', flexWrap: 'wrap' }}>
            <span style={{ fontSize: '0.8rem', color: meta.color }}>
              Stock: <strong>{need.currentStock}</strong>
            </span>
            {need.minStock > 0 && (
              <span style={{ fontSize: '0.8rem', color: meta.color }}>
                Mín: <strong>{need.minStock}</strong>
              </span>
            )}
            {need.maxStock > 0 && (
              <span style={{ fontSize: '0.8rem', color: meta.color }}>
                Máx: <strong>{need.maxStock}</strong>
              </span>
            )}
            {dailyRate && (
              <span style={{ fontSize: '0.8rem', color: meta.color }}>~{dailyRate}/día</span>
            )}
          </div>
        </div>
        ...
        >
          ...
          <div style={{ fontSize: '1.5rem', fontWeight: 800, color: '#2563eb', lineHeight: 1.1 }}>
        ...
        >
          ...
          >
            ...
            <ContextCell
              ...
              value={`${need.currentStock} u`}
              highlight={need.currentStock < 0}
            />
            <ContextCell label="Reservado" value={`${need.reservedQuantity} u`} />
            <ContextCell
              label="Mínimo configurado"
              value={need.minStock > 0 ? `${need.minStock} u` : '—'}
            />
            <ContextCell
              label="Máximo configurado"
              value={need.maxStock > 0 ? `${need.maxStock} u` : '—'}
            />
            <ContextCell
              label="Consumo 7 días"
              value={need.consumption7d > 0 ? `${need.consumption7d} u` : '—'}
            />
            {need.recentWaste48h > 0 && (
              <ContextCell label="Merma 48h" value={`${need.recentWaste48h} u`} highlight />
            )}
            <ContextCell
              label="Días restantes"
              value={need.daysOfStockRemaining !== null ? `${need.daysOfStockRemaining}d` : '—'}
            />
          ...
          <div
            style={{
              background: need.priority <= 2 ? '#fee2e2' : '#fef9c3',
              border: `1px solid ${need.priority <= 2 ? '#fca5a5' : '#fde68a'}`,
              borderRadius: '8px',
              padding: '0.875rem',
              marginBottom: '1rem',
            }}
          >
            <p
              style={{
                ...
                fontSize: '0.875rem',
                fontWeight: 500,
                color: need.priority <= 2 ? '#7f1d1d' : '#78350f',
              }}
            >
            ...
            <p
              style={{
                margin: '0.375rem 0 0',
                fontSize: '0.85rem',
                color: need.priority <= 2 ? '#991b1b' : '#92400e',
                lineHeight: 1.5,
              }}
            >
  ...
  return (
    <div
      style={{
        ...
        borderRadius: '6px',
        padding: '0.5rem 0.75rem',
        border: highlight ? '1px solid #fca5a5' : '1px solid #f3f4f6',
      }}
    >
      <div
        style={{
...
export function PurchasingPage() {
  ...
  const handleAddToList = async (need: ReplenishmentNeed) => {
    ...
    try {
      ...
      setAddingProductIds((p) => {
        const s = new Set(p);
        s.delete(need.productId);
        return s;
      });
    }
  };
  ...
  const handleDiscard = async (reason: string) => {
    ...
    if (discardTarget.need) {
      ...
      try {
        const result = (await PurchasingAPI.createRequest(selectedStoreId, {
        ...
        setDiscardingProductIds((p) => {
          const s = new Set(p);
          s.delete(n.productId);
          return s;
        });
      }
    }
  ...
  const handleStatusChange = async (req: PurchaseRequest, status: PurchaseStatus) => {
    ...
    try {
      ...
      const msgs: Record<string, string> = {
        bought: `${req.productName} marcado como comprado ✓`,
        postponed: `${req.productName} pospuesto`,
        pending: `${req.productName} reactivado`,
      };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      {/* ── Tabs ── */}
      <div
        style={{ display: 'flex', borderBottom: '2px solid var(--border)', alignItems: 'center' }}
      >
              ...
              <>
                ...
                <span
                  style={{
                    ...
                    borderRadius: '10px',
                    fontSize: '0.75rem',
                    background: needs.length > 0 ? '#fee2e2' : '#f3f4f6',
                    color: needs.length > 0 ? '#991b1b' : '#6b7280',
                    fontWeight: 700,
                  }}
                >
                  {needs.length}
              ...
              <>
                ...
                <span
                  style={{
                    ...
                    color: pendingRequests.length > 0 ? '#92400e' : '#6b7280',
                    fontWeight: 700,
                  }}
                >
                  {requests.filter((r) => r.status !== 'discarded').length}
                </span>
              </>
            )}
          </button>
          ...
          {loading ? (
            ...
            needs.map((need) => (
              <NeedCard
                key={need.productId}
                need={need}
                isInList={requests.some(
                  (r) => r.productId === need.productId && r.status === 'pending',
                )}
                    ...
                    <div style={{ flex: 1, minWidth: '160px' }}>
                      ...
                      <div style={{ fontSize: '0.8rem', color: '#6b7280', marginTop: '0.125rem' }}>
                        ...
                        {req.currentStock !== undefined && (
                          <>
                            {' '}
                            · Stock actual: <strong>{req.currentStock}</strong>
                          </>
                        )}
                        {req.minStock > 0 && (
                          <>
                            {' '}
                            · Mín: <strong>{req.minStock}</strong>
                          </>
                        )}
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '0.375rem', flexWrap: 'wrap' }}>
                      ...
                      {req.status !== 'bought' && (
                        <button
                          disabled={isActing}
                          onClick={() =>
                            setDiscardTarget({ id: req.id, storeId: selectedStoreId! })
                          }
                          style={{
                            padding: '5px 10px',
                            background: 'transparent',
      ...
      {discardTarget && (
        <DiscardModal
          productName={
            discardTarget.need?.productName ||
            requests.find((r) => r.id === discardTarget.id)?.productName ||
            ''
          }
