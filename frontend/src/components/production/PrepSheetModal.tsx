    ...
    <div className="modal-overlay">
      <div className="modal-content" style={{ maxWidth: '600px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
          <h3 style={{ margin: 0 }}>Hoja de Preparación (ORD-{order.id.toString().padStart(4, '0')})</h3>
          <button onClick={() => window.print()} style={{ background: 'transparent', border: '1px solid var(--border)', borderRadius: '4px', cursor: 'pointer', padding: '4px 8px', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        ...
        {loading ? (
          ...
          <div>
            <div style={{ display: 'flex', gap: '1rem', background: '#f8fafc', padding: '1rem', borderRadius: '8px', marginBottom: '1.5rem' }}>
              <div>
              ...
              <div>
                ...
                <div style={{ fontWeight: 500 }}>{order.storeName || 'Tienda #' + order.storeId}</div>
              </div>
            </div>
              ...
              <tbody>
                {sheet.ingredients.map(ing => (
                  <tr key={ing.productId} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '0.5rem 0', textAlign: 'right' }}>{ing.quantity} {ing.unit}</td>
                  </tr>
                ))}
                {sheet.ingredients.length === 0 && (
                  <tr><td colSpan={2} style={{ padding: '1rem 0', color: 'var(--muted)' }}>Esta orden no requiere ingredientes o no hay receta.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ padding: '0.5rem 1rem', background: 'var(--text)', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 500 }}>Cerrar</button>
        </div>
      </div>
    </div>
  );
