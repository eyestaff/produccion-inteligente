...
export function CreateOrderModal({ onClose, onSuccess }: { onClose: () => void, onSuccess: () => void }) {
  ...
  const [form, setForm] = useState({ storeId: '', businessLineId: '', productId: '', quantity: '' });

  useEffect(() => {
    Promise.all([CatalogAPI.getStores(), CatalogAPI.getBusinessLines(), CatalogAPI.getProducts()])
      .then(([s, l, p]) => {
        setStores(s || []); setLines(l || []); setProducts(p || []);
      ...
      <div className="modal-content">
        ...
        {loading ? (
          ...
          <form onSubmit={handleSubmit}>
            <label style={{ display: 'block', fontWeight: 500, marginBottom: '0.25rem' }}>Tienda</label>
            <select value={form.storeId} onChange={e => setForm({ ...form, storeId: e.target.value })}>
              {stores.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
            </select>

            <label style={{ display: 'block', fontWeight: 500, marginBottom: '0.25rem' }}>Línea de Negocio</label>
            <select value={form.businessLineId} onChange={e => setForm({ ...form, businessLineId: e.target.value })}>
              {lines.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}
            </select>

            <label style={{ display: 'block', fontWeight: 500, marginBottom: '0.25rem' }}>Producto a Fabricar</label>
            <select value={form.productId} onChange={e => setForm({ ...form, productId: e.target.value })}>
              {products.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>

            <label style={{ display: 'block', fontWeight: 500, marginBottom: '0.25rem' }}>Cantidad (Unidades)</label>
            <input type="number" min="1" value={form.quantity} onChange={e => setForm({ ...form, quantity: e.target.value })} placeholder="Ej: 500" />
            ...
            <div style={{ display: 'flex', gap: '1rem', marginTop: '1.5rem', justifyContent: 'flex-end' }}>
              ...
              <button type="submit" disabled={submitting} style={{ padding: '0.5rem 1rem', background: 'var(--text)', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 500 }}>
