...
export class AssetsService {
  ...
  private getKey(key: string): string {
    return `company_${this.companyId}/${key}`;
  }

  async listAssets(query?: string, page = 1, limit = 10): Promise<PaginatedAssets> {
    const prefixQuery = query ? `company_${this.companyId}/${query}` : `company_${this.companyId}/`;
    ...
    const strippedAssets = res.assets.map((a) => ({
      ...a,
      key: a.key.replace(`company_${this.companyId}/`, ''),
    }));
