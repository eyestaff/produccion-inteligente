...
export async function initializeDb(db: D1Database) {
  await db.exec(
    `CREATE TABLE IF NOT EXISTS records (id INTEGER PRIMARY KEY AUTOINCREMENT, company_id INTEGER, name TEXT, value TEXT, created_at TEXT, FOREIGN KEY(company_id) REFERENCES companies(id));
