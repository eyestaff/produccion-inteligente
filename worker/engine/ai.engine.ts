import { ForecastDashboardResult, ForecastRecommendation } from './forecast.engine';
...
export async function enhanceForecastWithAI(
  ...
  const recsToAnalyze = forecastResult.recommendations.filter(r => r.suggestedQuantity > 0 || r.metrics.recentWaste > 0);
...
No devuelvas ningún otro texto, solo el JSON.`;
  ...
  try {
    ...
    if (aiResult && aiResult.adjustments && Array.isArray(aiResult.adjustments)) {
      const adjustmentMap = new Map(aiResult.adjustments.map((a: any) => [a.id, a]));
      
      const newRecommendations = forecastResult.recommendations.map(r => {
        const adj = adjustmentMap.get(r.productId);
        if (adj && typeof adj.adjustedDemand === 'number' && adj.adjustedDemand !== r.suggestedQuantity) {
          return {
            ...r,
