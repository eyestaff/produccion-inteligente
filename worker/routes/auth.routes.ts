...
export async function handleAuthRoute(
  ...
  if (pathname === '/api/auth/reset-password' && request.method === 'POST') {
    try {
      ...
      const tokenHash = await hashPasswordResetToken(token);
      const resetToken = await findValidPasswordResetToken(env.DB, tokenHash, Date.now());

      if (!resetToken) {
        return new Response(JSON.stringify({ error: 'El enlace es inválido o ha caducado' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json' },
        });
      }
